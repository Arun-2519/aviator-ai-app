...
st.sidebar.info(
    "Upload a screenshot from the Aviator game. The system will extract all crash multipliers (most recent at top), and make a prediction from that batch."
)
...
def predict_bet_advice(history, confidence_threshold):
    ...
    recent = history[:10]  # most recent 10 rounds (topmost in image)
...
uploaded_file = st.file_uploader("Upload a clear Aviator Game Screenshot (PNG/JPG/JPEG) with multipliers visible (newest at the top of the list)", type=["png", "jpg", "jpeg"])
if uploaded_file:
...
if st.button("Clear All Data"):
    ...
    st.write("Data cleared. Ready for a new session!")
